  ...
  useEffect(() => {
    ...
    async function loadResults() {
      ...
      try {
        ...
        if (isMounted) {
          setError(err instanceof ApiError ? err.message : "Unable to load marks.");
        }
      } finally {
        if (isMounted) {
          setLoading(false);
  ...
  return (
    <DashboardLayout role="student">
      <div className="space-y-6">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-900 dark:text-white">Grades & Marks</h1>
          <p className="text-sm text-slate-500 dark:text-slate-400">View your graded exams and detailed feedback.</p>
        </div>
        ...
        {loading ? (
          ...
          <EmptyState title="No marks yet" description="Your marks will appear once evaluations are completed." />
        ) : (
